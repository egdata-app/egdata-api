...
Use `POST /search/natural-language` to describe the kind of game or content you want instead of composing keyword filters. The API embeds the query, ranks the offer vectors, and then loads the current offer documents from MongoDB. Vector metadata is not used as the source of offer data.
...
`query` is required and may contain up to 500 characters. `topK` is optional, defaults to `10`, and accepts values from `1` through `100`.
