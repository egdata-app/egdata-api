import { Item, Asset, Build, Offer, Changelog } from '../../models/index.js';
...
const resolvers: IResolvers<any, Context> = {
    Query: {
        ...
        items: async (_, { limit = 10, page = 1 }) => {
            ...
            const elements = await Item.find({}, undefined, { limit, skip, sort: { lastModifiedDate: -1 } }).lean();
    ...
    Item: {
        ...
        builds: async (parent, _, { loaders }) => {
            ...
            const appIds = parent.releaseInfo.map((r: any) => r.appId).filter(Boolean);
            if (appIds.length === 0) return [];
            return loaders.build.loadMany(appIds);
        },
        offers: async (parent) => {
            // Find offers that contain this item
            return Offer.find({ "items.id": parent.id }).lean();
        },
        mainOffer: async (parent) => {
            // Logic from /items/:id/offer
            const subItems = await db.collection("offersubitems").find({ "subItems.id": parent.id }, { projection: { _id: 1 } }).toArray();
            const offerIds = subItems.map(s => s._id);
            const offers = await Offer.find({ id: { $in: offerIds }, offerType: "BASE_GAME" }).lean();
            if (offers.length === 0) return null;
            return offers.find(o => !o.prePurchase) || offers[0];
        },
        changelog: async (parent, { limit = 10, page = 1 }) => {
            ...
            const builds = await db.db.collection("builds").find({ appName: { $in: assetIds } }).toArray();
